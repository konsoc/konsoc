function initData(sLocale) {
	...
	function requestRenderData() {
		...
		$('#btn_map_date_more').click(function () {
			var dtRng = document.getElementById("dateRange");
			if(dtRng.value < dtRng.max)
			{
				dtRng.value = Number(dtRng.value) + 1;
				chageMapDate();
			}
		});		
		
		$('#btn_map_date_less').click(function () {
			var dtRng = document.getElementById("dateRange");
			if(dtRng.value > dtRng.min)
			{	
				dtRng.value = Number(dtRng.value) - 1;
				chageMapDate();
			}
		...
		//var btn//= document.getElementById("dateRange");
