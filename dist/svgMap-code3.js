...
function svgMapWrapper(svgPanZoom) {
  ...
  svgMap.prototype.createMap = function () {
    ...
    if (!this.options.countries.EH) {
      mapPaths.MAR.d = mapPaths['MAR-ESH'].d;
      delete mapPaths.ESH;
    }
    delete mapPaths['MAR-ESH'];
  ...
  svgMap.prototype.clicked = function () {};

  svgMap.prototype._clicked = function (countryID) {
    this.clicked(countryID);
  }
    ...
    if (this.options.hideFlag === false) {
      ...
      if (this.options.flagType === 'image') {
        this.createElement(
          ...
          'src',
          this.options.flagURL.replace('{0}', code3_2[countryID].toLowerCase())
        );
